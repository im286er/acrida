    ...
    private SpannableStringBuilder addClickablePart(final Context context,
        ...
        if (getIsLike() == 1) {
            ...
            for (int i = 0; i < getLikeUser().size(); i++) {
                if (getLikeUser().get(i).getId() == ApplicationLoader.getInstance()
                        .getLoginUid()) {
                    getLikeUser().remove(i);
                }
            }
            getLikeUser().add(0, ApplicationLoader.getInstance().getLoginUser());
        }
        ...
        // 第一个赞图标
        // ImageSpan span = new ImageSpan(ApplicationLoader.getInstance(),
