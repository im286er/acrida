 ...
 */
public class RtfTweetsFragment extends InjectListFragment<Tweet, TweetsList> implements
        OnItemLongClickListener, OnTabReselectListener {

    protected static final String TAG = RtfTweetsFragment.class.getSimpleName();
    ...
    @Override
    protected Observable<retrofit2.adapter.rxjava.Result<TweetsList>> obtainData() {
        Bundle bundle = getArguments();
        if (bundle != null) {
            String str = bundle.getString("topic");
            if (str != null) {
               return mOSChinaService.getTweetTopicList(str, mCurrentPage, getPageSize());
            }
        }
        return mOSChinaService.getTweetList(mCatalog, mCurrentPage, getPageSize());
    }
