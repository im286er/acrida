...
import net.gility.acrida.content.TweetDetail;
...
import net.gility.acrida.ui.widget.RecordButtonUtil;
        ...
        mRlRecordSound.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mTweet != null) {
                    ...
                    ApplicationLoader.showToast("找不到语音动弹,可能已经被主人删除了");
                }
            }
        });
    ...
    private void setLikeState() {
        if (mTweet != null) {
            if (mTweet.getIsLike() == 1) {
                mTvLikeState.setTextColor(ApplicationLoader.getInstance().getResources().getColor(R.color.day_colorPrimary));
            } else {
                mTvLikeState.setTextColor(ApplicationLoader.getInstance().getResources().getColor(R.color.gray));
            }
        }
    }
        ...
        if (ApplicationLoader.getInstance().isLogin()) {
            if (mTweet.getIsLike() == 1) {
                ...
                mTweet.getLikeUser().add(0,
                        ApplicationLoader.getInstance().getLoginUser());
                mTweet.setLikeCount(mTweet.getLikeCount() + 1);
                OSChinaApi
            ...
            ApplicationLoader.showToast("先登陆再点赞~");
            UIHelper.showLoginActivity(getActivity());
        }
    }
        ...
        try {
            if (outAty.emojiFragment.getEditText().getTag() != null) {
                ...
                OSChinaApi.replyComment(mTweetId, CommentList.CATALOG_TWEET,
                        comment.getId(), comment.getAuthorId(), ApplicationLoader
                                .getInstance().getLoginUid(), str.toString(),
                        mCommentHandler);
            } else {
                OSChinaApi.publicComment(CommentList.CATALOG_TWEET, mTweetId,
                        ApplicationLoader.getInstance().getLoginUid(), str.toString(),
                        0, mCommentHandler);
            }
        } catch (Exception e) {
        }
