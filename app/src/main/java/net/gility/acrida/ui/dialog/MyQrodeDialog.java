...
import net.gility.acrida.utils.QrCodeUtils;
    ...
    private MyQrodeDialog(Context context, int defStyle) {
        ...
        try {
            bitmap = QrCodeUtils.Create2DCode(String.format(
                    "http://my.oschina.net/u/%s", ApplicationLoader.getInstance()
                            .getLoginUid()));
        ...
        mIvCode.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                if (FileUtils.bitmapToFile(bitmap,
                    ...
                    ApplicationLoader.showToast("二维码已保存到oschina文件夹下");
                } else {
                    ApplicationLoader.showToast("SD卡不可写，二维码保存失败");
                }
                return false;
            }
        });
