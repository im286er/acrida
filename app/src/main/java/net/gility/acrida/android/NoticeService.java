...
import net.gility.acrida.content.NoticeDetail;
...
public class NoticeService extends Service {
    ...
    private void notification(Notice notice) {
        ...
        if (ApplicationLoader.get(AppConfig.KEY_NOTIFICATION_SOUND, true)) {
            builder.setSound(Uri.parse("android.resource://"
                    + ApplicationLoader.getInstance().getPackageName() + "/"
                    + R.raw.notificationsound));
        }
        if (ApplicationLoader.get(AppConfig.KEY_NOTIFICATION_VIBRATION, true)) {
        ...
        public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
            try {
                ...
                if (notice != null) {
                    ...
                    if (ApplicationLoader.get(AppConfig.KEY_NOTIFICATION_ACCEPT, true)) {
                        notification(notice);
                    }
                    mNotice = notice;
                } else {
...
//                	ApplicationLoader appContext = ApplicationLoader.getInstance();
//                	if (appContext != null) {
//                	    appContext.Logout();
//                	}
//                    }
