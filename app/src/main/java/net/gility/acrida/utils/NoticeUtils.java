...
import net.gility.acrida.android.INoticeService;
...
public class NoticeUtils {
    ...
    public static void clearNotice(int type) {
        if (sService != null) {
            try {
                sService.clearNotice(ApplicationLoader.getInstance().getLoginUid(),
                        type);
    ...
    public static void tryToShutDown(Context context) {
        if (ApplicationLoader.get(AppConfig.KEY_NOTIFICATION_DISABLE_WHEN_EXIT, true)) {
            context.sendBroadcast(new Intent(
